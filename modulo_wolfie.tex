...
\begin{Estructura}{wolfie}[estr]
    \begin{Tupla}[estr]
      ...
      \tupItem{clientes}{array\_dimensionable(tuplaCarteraCliente)}
      
      \tupItem{\'ultimoLlamado}{<$cliente$: cliente, $promPorTitulo$:diccTrie(nombre, promesa)>)}%
    \end{Tupla}
    ...
    \begin{Tupla}[tuplaCarteraCliente]
    	\tupItem{cliente}{cliente,} 
    	\tupItem{t\'itulosDeCliente}{diccTrie(nombre, <$cantAcc$: nat, $promVenta$: *promesa>)}
    \end{Tupla}
    \indent\indent Con un orden definido por $a$<$b$ $\Leftrightarrow$ $a.cliente$ < $b.cliente$
