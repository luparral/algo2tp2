...
\begin{lstlisting}[mathescape]
 if $\neg$(e.ultimoLlamado.cliente = c $\land$ e.ultimoLlamado.fueUltimo) then		O(1)
   $\textbf{itClaves(diccString(nombre, infoTitulo))}$ it $\leftarrow$ CrearIt(e.titulos)							O(1)
...
\begin{lstlisting}[mathescape]
  ...
  if s.cot < cot then								O(1)
  	s.enAlza $\leftarrow$ true 								O(1)
  else 
  	s.enAlza $\leftarrow$ false								O(1)
  fi
  s.cot $\leftarrow$ cot									O(1)
...
\end{lstlisting}
\textbf{Complejidad:} O(|nt|)+5*O(1)+C*4*O(1)+O(C)+O(C)+O(C(log(C)))+O(1)+C*4*O(1)+O(C)+O(C(log(C)))=\\\indent\indent\indent O(|nt|+C(log(C)))
