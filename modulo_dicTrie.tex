...
\textsc{Heapsort}(\textbf{in} arreglo(tuplas): a, \textbf{in} int: n) $\rightarrow$ \textbf{res} = arreglo(tuplas)
\begin{lstlisting}[mathescape]
 fin $\leftarrow$ (n-1)
 while (end > 0)
 	swap(a[fin], a[0])
...
\textsc{Heapificar}(\textbf{in} arreglo(tuplas): a, \textbf{in} int: n) $\rightarrow$ \textbf{res} = arreglo(tuplas)
\begin{lstlisting}[mathescape]
...
\textsc{Bajar}(\textbf{in} arreglo(tuplas): a, \textbf{in} int: comienzo, \textbf{in} int: fin) $\rightarrow$ \textbf{res} = arreglo(tuplas)
\begin{lstlisting}[mathescape]
 int: raiz
 int: hijo
 int: pasaMano
...
\textsc{BusquedaBinaria}(\textbf{in} arreglo(tuplas): a, nat: cliente, nat: tam)
\begin{lstlisting}[mathescape]
