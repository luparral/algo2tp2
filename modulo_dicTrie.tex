...
iCrearDicc() $\rightarrow$ res = diccString($\alpha$)
...
iCrearNodo() $\rightarrow$ res = nodo
\begin{lstlisting}[mathescape]
...
iDefinir(inout diccString($\alpha$): d, in string: c, in alfa: s)
\begin{lstlisting}[mathescape]
 i $\leftarrow$ 0
 p $\leftarrow$ d.raiz
 while (i < (longitud(s)))
...
iSignificado(in diccString($\alpha$): d, in string: c) $\rightarrow$ res = alfa
\begin{lstlisting}[mathescape]
 i $\leftarrow$ 0
 p $\leftarrow$ d.raiz
 while (i < (longitud(s)))
...
iDefinido?(in diccString: d, in string: c) $\rightarrow$ res = bool
\begin{lstlisting}[mathescape]
 i $\leftarrow$ 0
 p $\leftarrow$ d.raiz
 while (i < (longitud(s)))
...
iClaves(in diccString: d) $\rightarrow$ res = lista\_enlazada(string)
\begin{lstlisting}[mathescape]
return itClaves(d)
\end{lstlisting}
